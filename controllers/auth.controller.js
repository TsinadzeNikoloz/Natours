...
exports.login = catchAsync(async (req, res, next) => {
	...
	// 3) If everything is good, send jwt token to client
...
exports.jwtProtect = catchAsync(async (req, res, next) => {
	...
	// 2) Verify token
	const decoded_token = await util.promisify(jwt.verify)(
		token,
	...
	if (current_user.changedPasswordAfter(decoded_token.iat)) {
		return next(
			new AppError(
...
exports.updatePassword = catchAsync(async (req, res, next) => {
	...
	await user.save(); // will run validators + pre('save') middleware
	// ⚠️ Don't use findByIdAndUpdate! Middleware won't run.
